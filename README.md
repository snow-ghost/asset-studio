...
Открыть http://localhost:5190. Выбрать тип ассета, нажать «New placeholder» или «Import» (`.glb`,
самодостаточный `.gltf` — для моделей; `.png` — для вида texture), подвинуть модель гизмо (W/E/R) или
числами в панели справа, кликнуть меш и поправить цвет, metalness, roughness и текстуру базового цвета из
текстурных ассетов студии, «Save». Новая текстура — процедурная (шахматка, полосы, шум) с параметрами в
панели; её рецепт хранится в метаданных ассета и редактируется после открытия. Ctrl+Z / Ctrl+Shift+Z —
отмена и возврат. Сохранённые ассеты появляются слева; клик — открыть. Несохранённые правки помечены
«● modified», и студия спросит, прежде чем их потерять.
...
M2 (текстуры) — импорт PNG, процедурные текстуры с рецептом в метаданных, назначение на материал, round-trip
по пикселям — спека [specs/002-textures](specs/002-textures/spec.md), отчёт [docs/M2-REPORT.md](docs/M2-REPORT.md).
Дальше — рельеф и вегетация (M3), предметы, персонажи и загрузчик манифеста в клиенте wowd (см. roadmap). Правила работы — в [AGENTS.md](AGENTS.md).
